...
from apps.employment.schemas import (
    ...
    TgMatchContextRequest,
    TgMatchStatusUpdateRequest,
...
@router.post("/reactions/by-context", response_model=ReactionResponse)
async def reactions_by_context(
    schema: TgReactionContextRequest,
    db: AsyncSession = Depends(get_db),
    idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"),
):
    if not idempotency_key:
        raise HTTPException(status_code=400, detail="Idempotency-Key header is required")
    reaction, match, replay = await EmploymentService.submit_reaction_by_tg_context(db, schema, idempotency_key)
    return ReactionResponse(
        reaction_id=reaction.id,
        match_id=match.id if match else None,
        match_status=match.status if match else None,
...
@router.patch("/matches/status-by-context", response_model=MatchResponse)
async def patch_match_status_by_context(schema: TgMatchStatusUpdateRequest, db: AsyncSession = Depends(get_db)):
    return await EmploymentService.update_match_status_by_tg_context(
        db,
        role=schema.role,
