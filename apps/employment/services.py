...
from apps.employment.enums import EntityType, MatchStatus, ProfileChangeSource, ReactionAction, ReactionSource, VacancyStatus
...
class EmploymentService:
    ...
    @staticmethod
    async def submit_reaction_by_tg_context(
        db: AsyncSession, schema: TgReactionContextRequest, idempotency_key: str
        ...
        if schema.role == EntityType.CANDIDATE:
            if not tg.linked_candidate_id:
                raise HTTPException(status_code=400, detail="Telegram is not linked to candidate")
            if schema.vacancy_id is None:
                raise HTTPException(status_code=400, detail="vacancy_id is required for candidate reactions")

            vacancy = (await db.execute(select(Vacancy).where(Vacancy.id == schema.vacancy_id))).scalars().first()
            if not vacancy:
                raise HTTPException(status_code=404, detail="Vacancy not found")

            reaction_schema = ReactionRequest(
                ...
                source=ReactionSource.TELEGRAM_BOT,
            )
            return await EmploymentService.submit_reaction(db, reaction_schema, idempotency_key)

        # organization path by vacancy context only (candidate selected from pending inbound likes)
        if not tg.linked_organization_id:
            raise HTTPException(status_code=400, detail="Telegram is not linked to organization")
        if schema.vacancy_id is None:
            raise HTTPException(status_code=400, detail="vacancy_id is required for organization reactions")

        inbound_q = (
            select(EmploymentReaction)
            .where(
                and_(
                    EmploymentReaction.initiator_entity_type == EntityType.CANDIDATE,
                    EmploymentReaction.target_entity_type == EntityType.ORGANIZATION,
                    EmploymentReaction.target_entity_id == tg.linked_organization_id,
                    EmploymentReaction.vacancy_id == schema.vacancy_id,
                    EmploymentReaction.action == ReactionAction.LIKE,
                )
            )
            .order_by(EmploymentReaction.created_at.desc())
            .limit(10)
        )
        inbound = list((await db.execute(inbound_q)).scalars().all())
        target_candidate_id = None
        for candidate_like in inbound:
            existing_org_reaction = (
                await db.execute(
                    select(EmploymentReaction).where(
                        and_(
                            EmploymentReaction.initiator_entity_type == EntityType.ORGANIZATION,
                            EmploymentReaction.initiator_entity_id == tg.linked_organization_id,
                            EmploymentReaction.target_entity_type == EntityType.CANDIDATE,
                            EmploymentReaction.target_entity_id == candidate_like.initiator_entity_id,
                            EmploymentReaction.vacancy_id == schema.vacancy_id,
                        )
                    )
                )
            ).scalars().first()
            if not existing_org_reaction:
                target_candidate_id = candidate_like.initiator_entity_id
                break

        if not target_candidate_id:
            raise HTTPException(status_code=404, detail="No pending candidate reaction found for vacancy")

        reaction_schema = ReactionRequest(
            initiator_entity_type=EntityType.ORGANIZATION,
            initiator_entity_id=tg.linked_organization_id,
            target_entity_type=EntityType.CANDIDATE,
            target_entity_id=target_candidate_id,
            vacancy_id=schema.vacancy_id,
            action=schema.action,
            source=ReactionSource.TELEGRAM_BOT,
        )
    ...
    async def submit_reaction(db: AsyncSession, schema: ReactionRequest, idempotency_key: str) -> tuple[EmploymentReaction, EmploymentMatch | None, bool]:
        ...
        request_hash = sha256(json.dumps(schema.model_dump(mode="json"), sort_keys=True, ensure_ascii=False).encode("utf-8")).hexdigest()
    ...
    @staticmethod
    async def get_match_by_tg_context(db: AsyncSession, role: EntityType, tg_user_id: str, vacancy_id: int) -> EmploymentMatch:
    ...
    @staticmethod
    async def update_match_status_by_tg_context(
        db: AsyncSession, role: EntityType, tg_user_id: str, vacancy_id: int, status: MatchStatus
    ) -> EmploymentMatch:
        match = await EmploymentService.get_match_by_tg_context(db, role, tg_user_id, vacancy_id)
        return await EmploymentService.update_match_status(db, match.id, MatchStatusUpdateRequest(status=status))
