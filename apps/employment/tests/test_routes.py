...
@pytest.mark.asyncio
async def test_patch_match_status_by_context(monkeypatch):
    app = FastAPI()
    app.include_router(routes.router)
    monkeypatch.setattr(
        routes.EmploymentService,
        "update_match_status_by_tg_context",
        AsyncMock(return_value=SimpleNamespace(id=1, candidate_id=10, organization_id=20, vacancy_id=30, status=MatchStatus.CLOSED, matched_at=None)),
    )

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.patch(
            "/employment/matches/status-by-context",
            json={"role": "candidate", "tg_user_id": "123", "vacancy_id": 30, "status": "closed"},
        )
...
@pytest.mark.asyncio
async def test_reaction_by_context_candidate_payload(monkeypatch):
    app = FastAPI()
    app.include_router(routes.router)
    monkeypatch.setattr(
        routes.EmploymentService,
        "submit_reaction_by_tg_context",
        AsyncMock(return_value=(SimpleNamespace(id=12), SimpleNamespace(id=3, status=MatchStatus.MUTUAL_MATCHED), False)),
    )
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.post(
            ...
            json={"role": "candidate", "telegram_id": "123", "vacancy_id": 10, "action": "like"},
        )
